...
import { expireStaleCreditsForCustomer } from "@/server/credits";
...
async function applyPlanAllocations(options: {
  ...
  for (const allocation of plan.allocations) {
    const bucket = resolveAllocationBucket(allocation.bucket, shouldDeferCurrent);
    ...
    entries.push({
      ...
      occurredAt: computeAllocationDate(bucket, allocation, purchasedAt),
...
function computeAllocationDate(
  bucket: CreditBucket,
  allocation: PlanCreditAllocation,
  purchasedAt: Date,
) {
  if (bucket === "CURRENT") {
    return purchasedAt;
...
function resolveAllocationBucket(
  allocationBucket: PlanCreditAllocation["bucket"],
  shouldDeferCurrent: boolean,
): CreditBucket | null {
  if (allocationBucket === "CURRENT") {
    return shouldDeferCurrent ? null : "CURRENT";
  }

  if (allocationBucket === "NEXT") {
    return "NEXT";
  }

  if (allocationBucket === "IMMEDIATE") {
    return shouldDeferCurrent ? "NEXT" : "CURRENT";
  }
