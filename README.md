...
2. `STRIPE_SECRET_KEY` と `STRIPE_WEBHOOK_SECRET` を設定します。Webhook 秘密鍵はカンマ区切りで複数指定できるため、Vercel の再デプロイ時に Stripe 側で新しいエンドポイントを発行した場合は、古い値と新しい値の両方を入れておくと切り替え期間中もイベントを受信できます。
3. Stripe Webhook を `https://{YOUR_DOMAIN}/api/stripe/webhook` に向け、`checkout.session.completed` / `invoice.paid` / `customer.subscription.updated` / `customer.subscription.deleted` のイベントを購読します。（互換用に `https://{YOUR_DOMAIN}/api/stripe-webhook` でも同じエンドポイントへ転送されます）本番運用では `https://<project>.vercel.app` やカスタムドメインなど、デプロイごとに変わらない URL を指定してください。プレビュー環境で個別の URL を利用する場合でも、Webhook 秘密鍵をカンマ区切りで追加すれば同じデプロイから複数のエンドポイントを許可できます。
